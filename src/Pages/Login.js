...
export function FormularioDeIngreso() {
  const html = `
      ...
      <input type="password"  name="password" placeholder ="Contraseña" required id="PasswordUser">
      <img src="" id="CampoVacioPassword" class="error"> 
      </div>
      
      <button type="submit" id="login" class="btn" > Iniciar sesion <a href="#/Register"> </a> </button>
      <form> 
      <div>`;
...
export function DatosDeLogin() {
  ...
  mostrarPassword.addEventListener('keydown', () => { // Eliminamos su type del input
    contrasena.removeAttribute('type');
  });
  mostrarPassword.addEventListener('keyup', () => { // Agregamos type de input
